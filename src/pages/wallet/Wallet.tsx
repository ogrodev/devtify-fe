...
export default function Wallet() {
	...
	useEffect(() => {
		updateAppSettings("FETCH_USER_INVENTORY");
			...
			<div className={styles.position}>
				<div className={`d-flex justify-content-between align-items-center ${styles.position}`}>
					<div className={styles.titleContainer}>
						<h2>
							<BiStore size="1.5em" /> Your Wallet
						</h2>
						<span>Here you can check the items that you own</span>
					</div>
				...
				<div className={`d-flex gap-3 flex-wrap align-content-stretch ${styles.position}}`}>
					{!!settings.inventory?.length &&
						settings.inventory?.map((product) => {
							return <ProductCard product={product} key={product.id} />;
						})}
					{!settings.inventory?.length && <Spinner />}
				</div>
				<div className="pt-5 mt-5" />
				<div className="my-5">
					<div className="mb-3">
						<h5>Our Wallet continues to grow</h5>

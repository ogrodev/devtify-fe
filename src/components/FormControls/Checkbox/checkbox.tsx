					...
					<>
						<label
							className={`${props.className} ${
								props.type && props.type === "radio" ? styles.radio : styles.checkbox
							}`}
							aria-invalid={formState.isSubmitted && formState.errors[props.id] ? "true" : "false"}
						>
							<input
								className="mx-2"
							...
							<svg viewBox="0 0 21 18">
								<symbol
									id={"tick-path" + (props.value || props.id)}
									viewBox="0 0 21 18"
								...
								<defs>
									<mask id={"tick" + (props.value || props.id)}>
										<use
											className={styles.tick + " " + styles.mask}
											href={`#tick-path${props.value || props.id}`}
										/>
									</mask>
								</defs>
								<use
								...
								<path
									fill="white"
									mask={`url(#tick${props.value || props.id})`}
						...
						<span
							style={props.showError ? { display: "block" } : { display: "none" }}
